...
namespace Veilborne.Camera
{
    public class FpsCameraController : ICameraController
    {
        ...
        public FpsCameraController(IInputProvider input, float moveSpeed = 7.5f, float mouseSensitivity = 0.0035f)
        {
            _input = input;
        ...
        public Vector3 UpdateAndGetHorizontalMove(CameraComponent camera, float dt)
        {
            ...
            if (_input.IsKeyDown(InputKeys.KEY_W)) horizMoveDir += flatForward;
            if (_input.IsKeyDown(InputKeys.KEY_S)) horizMoveDir -= flatForward;
            if (_input.IsKeyDown(InputKeys.KEY_A)) horizMoveDir -= flatRight;
            if (_input.IsKeyDown(InputKeys.KEY_D)) horizMoveDir += flatRight;
